    ...
    if st.session_state.get('mobile_data') is None:
        ...
        if fpath.exists():
            try:
                ...
                pass
    # Borzo — monthly overall
    if st.session_state.get('borzo_monthly_data') is None:
        fpath = app_dir / "data" / "borzo_monthly_latest.xlsx"
        if fpath.exists():
            try:
                from data_loader import load_borzo_monthly
                with open(fpath, 'rb') as f:
                    fb = f.read()
                st.session_state['borzo_monthly_data'] = load_borzo_monthly(fb)
            except Exception:
                pass
    # Borzo — per-client
    if st.session_state.get('borzo_clients_data') is None:
        fpath = app_dir / "data" / "borzo_clients_latest.xlsx"
        if fpath.exists():
            try:
                from data_loader import load_borzo_clients
                with open(fpath, 'rb') as f:
                    fb = f.read()
                st.session_state['borzo_clients_data'] = load_borzo_clients(fb)
            except Exception:
                pass
...
def require_data() -> pd.DataFrame:
    ...
    # Use explicit None checks — 'or' causes ValueError on DataFrames
...
def require_ev_data() -> pd.DataFrame:
    """Return the EV Rental dataframe (Test EV Rental sheet) from session state, or halt."""
    _auto_load_from_data_folder()
    df = st.session_state.get('ev_data')
    if df is None or (hasattr(df, 'empty') and df.empty):
        st.warning("⚠️ No EV data available. Please ask your admin to publish data via the Updater page.")
        st.stop()
    return df.copy()


def get_ev_optional():
    """Return EV dataframe if loaded, else None."""
    _auto_load_from_data_folder()
    df = st.session_state.get('ev_data')
    if df is None or (hasattr(df, 'empty') and df.empty):
        return None
...
def get_mobile_optional():
    """Return Mobile dataframe if loaded, else None."""
    _auto_load_from_data_folder()
...
def build_mobile_trend(df: pd.DataFrame, group_cols: list[str], mode: str) -> pd.DataFrame:
    """Aggregate df by period for Mobile Sellers trend charts. Uses MAX for riders, SUM for others."""
    if mode == "Weekly":
        trend = (
            df.groupby(['Year', 'Week (by Year)'] + group_cols, observed=True)
            .agg(Cups=('Total Cups Sold', 'sum'),
                 ...
                 Riders=('Total Active Riders', 'max'),
                 COGS=('COGS', 'sum'),
...
# ── Borzo helpers ──────────────────────────────────────────────────────────────
def require_borzo_monthly() -> pd.DataFrame:
    _auto_load_from_data_folder()
    df = st.session_state.get('borzo_monthly_data')
    if df is None:
        st.warning("⚠️ No Borzo monthly data available. Please ask your admin to publish data via the Updater page.")
        st.stop()
    return df.copy()
...
def require_borzo_clients() -> pd.DataFrame:
    _auto_load_from_data_folder()
    df = st.session_state.get('borzo_clients_data')
    if df is None:
        st.warning("⚠️ No Borzo per-client data available. Please ask your admin to publish data via the Updater page.")
        st.stop()
    return df.copy()
...
def get_borzo_monthly_optional() -> pd.DataFrame | None:
    _auto_load_from_data_folder()
    return st.session_state.get('borzo_monthly_data')
...
def get_blitz_mobile_optional() -> pd.DataFrame | None:
    _auto_load_from_data_folder()
    return st.session_state.get('mobile_data')


# ── Group-level helpers ────────────────────────────────────────────────────────
def blitz_delivery_monthly(df: pd.DataFrame) -> pd.DataFrame:
    """Collapse Blitz Delivery weekly data to monthly Revenue/Cost/GP."""
    if df is None or df.empty:
        return pd.DataFrame()
    g = (df.groupby(['Year', 'Month'], observed=True)
    ...
    g['MonthNum'] = g['Month'].cat.codes + 1
    g['Stream'] = 'Blitz — Delivery'
    g['Company'] = 'Blitz'
    return g.sort_values(['Year', 'MonthNum'])


def blitz_mobile_monthly(df: pd.DataFrame) -> pd.DataFrame:
    """Collapse Blitz Mobile weekly data to monthly (Gross Revenue / Total Cost / Profit)."""
    if df is None or df.empty:
        return pd.DataFrame()
    g = (df.groupby(['Year', 'Month'], observed=True)
           .agg(Revenue=('Gross Revenue', 'sum'),
                Cost=('Total Cost (Mobile)', 'sum'),
                GP=('Profit Calc', 'sum'),
                Volume=('Total Cups Sold', 'sum'))
           .reset_index())
    g['Month'] = pd.Categorical(g['Month'], categories=MONTH_ORDER, ordered=True)
    g['MonthNum'] = g['Month'].cat.codes + 1
    g['Stream'] = 'Blitz — Mobile Sellers'
    g['Company'] = 'Blitz'
    return g.sort_values(['Year', 'MonthNum'])


def borzo_monthly_std(df: pd.DataFrame) -> pd.DataFrame:
    """Return Borzo monthly data in standard Revenue/Cost/GP/Volume schema."""
    if df is None or df.empty:
        return pd.DataFrame()
    g = df[['Year', 'Month', 'MonthNum', 'Revenue', 'Cost', 'GP', 'Orders']].copy()
    g = g.rename(columns={'Orders': 'Volume'})
    g['Stream'] = 'Borzo — 3PL'
    g['Company'] = 'Borzo'
    return g.sort_values(['Year', 'MonthNum'])
