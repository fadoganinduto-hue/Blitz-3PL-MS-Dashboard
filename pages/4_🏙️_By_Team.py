...
with _pc1:
    view_mode = period_selector(page_key="team")
with _pc2:
    period_picker(df, view_mode, page_key="team")
pop = pop_label(view_mode)

periods   = get_available_periods(df, view_mode)
curr_yr, curr_p, curr_lbl = selected_period_info(df, view_mode, page_key="team")
prev_info = prev_period_info(periods, curr_yr, curr_p)

curr_df = selected_period_df(df, view_mode, page_key="team")
...
for team in teams:
    ...
    client_agg = (
        team_df.groupby('Client Name', observed=True)
    ...
    client_agg['GP Margin %'] = np.where(
        client_agg['Revenue'] != 0,
        client_agg['GP'] / client_agg['Revenue'] * 100, 0
    )

    # Merge with previous period for PoP%
    if not prev_df.empty:
        prev_team_df = prev_df[prev_df['Blitz Team'] == team]
        prev_client = (
            prev_team_df.groupby('Client Name', observed=True)['GP']
            .sum().reset_index().rename(columns={'GP': 'GP_prev'})
        )
        client_agg = client_agg.merge(prev_client, on='Client Name', how='left').fillna(0)
        client_agg[f'GP {pop}%'] = client_agg.apply(
            lambda r: pop_pct(r['GP'], r['GP_prev']) if r['GP_prev'] != 0 else None, axis=1
        )
    else:
