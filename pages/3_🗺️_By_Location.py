...
with _pc1:
    view_mode = period_selector(page_key="loc")
with _pc2:
    period_picker(df, view_mode, page_key="loc")
pop = pop_label(view_mode)

periods   = get_available_periods(df, view_mode)
curr_yr, curr_p, curr_lbl = selected_period_info(df, view_mode, page_key="loc")
prev_info = prev_period_info(periods, curr_yr, curr_p)

curr_df = selected_period_df(df, view_mode, page_key="loc")
