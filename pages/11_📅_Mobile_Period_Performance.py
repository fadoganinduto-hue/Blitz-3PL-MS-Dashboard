...
metrics_data = [
    ...
    ("Active Riders", 'Total Active Riders', fmt_vol),
    ("Gross Revenue", 'Gross Revenue', fmt_idr),
    ("Blitz Revenue", 'Blitz Revenue', fmt_idr),
    ("Profit", 'Profit Calc', fmt_idr),
]

# Per-driver derived KPIs
cups_b = get_kpi_val(df_b, 'Total Cups Sold')
riders_b = get_kpi_val(df_b, 'Total Active Riders')
cpd_b = cups_b / riders_b if riders_b else 0
rpd_b = get_kpi_val(df_b, 'Gross Revenue') / riders_b if riders_b else 0
if lbl_a:
    cups_a_val = get_kpi_val(df_a, 'Total Cups Sold')
    riders_a_val = get_kpi_val(df_a, 'Total Active Riders')
    cpd_a = cups_a_val / riders_a_val if riders_a_val else 0
    rpd_a = get_kpi_val(df_a, 'Gross Revenue') / riders_a_val if riders_a_val else 0
else:
    cpd_a, rpd_a = 0, 0

metrics_data += [
    ("Cups / Driver", '_cpd', lambda x: f"{x:,.1f}"),
    ("Revenue / Driver", '_rpd', fmt_idr),
]
...
for metric_label, col, formatter in metrics_data:
    # Handle derived per-driver metrics
    if col == '_cpd':
        val_a, val_b = cpd_a, cpd_b
    elif col == '_rpd':
        val_a, val_b = rpd_a, rpd_b
    else:
...
if lbl_a:
    ...
    merged['Cups_A'] = merged['Total Cups Sold_A'] if 'Total Cups Sold_A' in merged.columns else 0
    merged['Cups_B'] = merged['Total Cups Sold_B'] if 'Total Cups Sold_B' in merged.columns else 0
    merged['BlitzRev_A'] = merged['Blitz Revenue_A'] if 'Blitz Revenue_A' in merged.columns else 0
    merged['BlitzRev_B'] = merged['Blitz Revenue_B'] if 'Blitz Revenue_B' in merged.columns else 0
    merged['Profit_A'] = merged['Profit Calc_A'] if 'Profit Calc_A' in merged.columns else 0
    merged['Profit_B'] = merged['Profit Calc_B'] if 'Profit Calc_B' in merged.columns else 0
