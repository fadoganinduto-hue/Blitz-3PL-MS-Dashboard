...
ev_leasing_mask = (
    (df_full.get('EV Reduction (3PL & KSJ)', pd.Series(0, index=df_full.index)).abs() > 0) |
    (df_full.get('EV Manpower', pd.Series(0, index=df_full.index)).abs() > 0) |
    (df_full.get('EV Revenue + Battery (Rental Client)', pd.Series(0, index=df_full.index)).abs() > 0)
)
