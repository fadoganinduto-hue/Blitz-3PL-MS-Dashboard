...
st.subheader("Borzo — Monthly (Overall)")
st.caption("Upload the Borzo Metrics Masterfile. Used for overall monthly trends & group comparisons.")
borzo_m_file = st.file_uploader(
    "Upload Borzo monthly masterfile (.xlsx)",
    type=['xlsx'],
    key="admin_borzo_monthly"
)

if borzo_m_file:
    st.write(f"**File:** {borzo_m_file.name}")
    st.write(f"**Size:** {borzo_m_file.size:,} bytes")
    if st.button("Publish Borzo Monthly Data", key="btn_borzo_m"):
        file_bytes = borzo_m_file.getvalue()
        success, result = commit_to_github(
            "data/borzo_monthly_latest.xlsx",
            file_bytes,
            "Admin: Update borzo_monthly_latest.xlsx"
        )
        if success:
            st.success("✅ Borzo monthly data committed to GitHub")
            st.info("Streamlit Cloud will redeploy your app within ~30 seconds.")
        else:
            st.error(f"Failed to commit: {result.get('message', 'Unknown error')}")
...
st.subheader("Borzo — Per-Client")
st.caption("Upload the Borzo 'Summary Penjualan' workbook (12 month tabs). Used for per-client drilldowns.")
borzo_c_file = st.file_uploader(
    "Upload Borzo per-client workbook (.xlsx)",
    type=['xlsx'],
    key="admin_borzo_clients"
)

if borzo_c_file:
    st.write(f"**File:** {borzo_c_file.name}")
    st.write(f"**Size:** {borzo_c_file.size:,} bytes")
    if st.button("Publish Borzo Per-Client Data", key="btn_borzo_c"):
        file_bytes = borzo_c_file.getvalue()
        success, result = commit_to_github(
            "data/borzo_clients_latest.xlsx",
            file_bytes,
            "Admin: Update borzo_clients_latest.xlsx"
        )
        if success:
            st.success("✅ Borzo per-client data committed to GitHub")
            st.info("Streamlit Cloud will redeploy your app within ~30 seconds.")
        else:
            st.error(f"Failed to commit: {result.get('message', 'Unknown error')}")
