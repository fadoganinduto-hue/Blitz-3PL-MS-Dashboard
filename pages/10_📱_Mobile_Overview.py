...
d1.metric("Active Riders", fmt_vol(riders_v), f"{riders_p:+.1f}% {pop}" if riders_p is not None else None)
...
d2.metric("Cups / Driver", f"{cups_per_driver:,.1f}", f"{cpd_pop:+.1f}% {pop}" if cpd_pop is not None else None)
...
d3.metric("Revenue / Driver", fmt_idr(rev_per_driver), f"{rpd_pop:+.1f}% {pop}" if rpd_pop is not None else None)
